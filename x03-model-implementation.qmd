...
The drop from random cv AUC (0.972) to spatial cv AUC (0.957) reflects spatial autocorrelation in the training data where nearby peat and non-peat observations are more similar and random folds
overestimate performance. 

# Gradient Boosting - XGBoost and LightGBM

XGBoost and LightGBM were trained on the 77 feature set using spatial block cv. Class imbalance was addressed with 'scale_pos_weight' set on the ratio of negative to positive training examples.
...
Random cv AUC for LightGBM (PROB_LGBM_001, before correction) was 0.9965 which was an overestimate caused by spatial autocorrelation and point_id leakage. 

# Generalized Additive Model (GAM)
A Logistic GAM was fit using the pygam library with thin-plate spline terms for each of the 76 features. Due to the O(n²) memory complexity of kernel smoothing, every
...
# Support Vector Machine (PROB_SVM_001)
...
# Convolutional Neural Network (PROB_CNN_001 through PROB_CNN_004)
...
The F1-optimal threshold of **0.327** was selected as the operational classification threshold. At this threshold, the RF model correctly identifies 85% of true peat observations
(sensitivity = 0.85) with a precision of 0.84, expanding the predicted peat area from 15.5% to 18.4% of training points compared to the 0.50 threshold. The Youden's J threshold (0.186) was
tested but not used as it predicted peat too often resulting in a false positive rate of ~25%.

The F1-optimal threshold of 0.327 was also used as the peat probability mask for the various peat property models that were only run on areas were peat is likely to exist
